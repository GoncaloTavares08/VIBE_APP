...
export function ManualCheckin({ onClose, embedded = false }: ManualCheckinProps) {
  ...
  const fetchGuests = async (query: string) => {
    setLoading(true);
    try {
      const response = await apiFetch(`/staff/scan/search?query=${encodeURIComponent(query)}&user_id=${userId}`);
  ...
  const handleCheckIn = async (guestId: string) => {
    try {
      const response = await apiFetch(`/staff/scan/manual-checkin?user_id=${userId}`, {
        method: 'POST',
