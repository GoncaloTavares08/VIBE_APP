    ...
    const handleSendCode = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                // If checking only for email existence without revealing explicitly, usually status is success.
                // But the backend responds with error "Email não encontrado" as per user request to "verify if email exists".
            }
        } catch (err) {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/controllers/auth.php`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'reset-password', email, password: passwords.new })
            });
            const data = await response.json();
