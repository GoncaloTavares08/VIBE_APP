...
export function Rewards() {
  ...
  const fetchRewards = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/controllers/admin_rewards_manage.php?action=list', {
        headers: {
  ...
  const toggleAvailability = async (id: number) => {
    try {
      const response = await fetch('/api/controllers/admin_rewards_manage.php?action=toggle_availability', {
        method: 'POST',
        headers: {
  ...
  const openEditModal = (reward: Reward) => {
    ...
    setImageFile(null);
    setImagePreview(reward.image_path ? `/api/controllers/serve_image.php?file=${reward.image_path}` : null);
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/controllers/admin_rewards_manage.php?action=${action}`, {
        method: 'POST',
        headers: {
          'X-Client-ID': getClubSlug()
        },
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch('/api/controllers/admin_rewards_manage.php?action=delete', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Client-ID': getClubSlug()
          ...
          {rewards.map((reward) => (
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                {reward.image_path ? (
                  <img
                    src={`/api/controllers/serve_image.php?file=${reward.image_path}`}
                    alt={reward.name}
